  ...
  :QWidget(parent)
{
  ...
  info_name_label=new QLabel(tr("Adapter:"),this);
...
void MainWidget::nameActivatedData(int n)
{
  ...
  default:
    ...
    if(hpi_mode[card]!=hpi_serial[card]) {
      ...
      fprintf(stderr,"%s %d\n",(const char *)str.toUtf8(),hpi_mode[card]);
    }
    break;
  }
}
...
{
  VirtDetails *d=new VirtDetails(hpi_indexes[info_name_box->currentIndex()],
				 hpi_profile[info_name_box->currentIndex()],
				 hpi_profile_quan[info_name_box->currentIndex()],
				 this);
