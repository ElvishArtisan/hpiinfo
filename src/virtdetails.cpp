  ...
  : QDialog(parent)
{
  ...
  setWindowTitle(tr("RdHPIInfo - Profile Details"));
  ...
  virt_profile_label=new QLabel(tr("Profile")+":",this);
  ...
  for(uint16_t i=0;i<profile_quan;i++) {
    if(HPI_ProfileGetName(NULL,profile,i,name,200)==0) {
      if(QString(name).toLower()!="unassigned") {
	virt_profile_box->insertItem(-1,name);
      }
    }
  }
